const STEPS = [
  ...
  },
  {
    ...
    questions: [
      ...
      { id: "familiar", label: "Sans chercher à analyser, cette sensation vous paraît-elle familière ?", type: "text", hint: "Un souvenir peut venir spontanément, mais il n’est pas nécessaire d’en trouver un." },
      { id: "pastNeed", label: "De quoi auriez-vous eu besoin à ce moment-là ?", type: "text" }
    ]
  },
  {
    title: "A — Axe essentiel",
    intro: "Nommer l’énergie centrale et le besoin sous-jacent, en laissant toujours la personne valider ou ajuster.",
    questions: [
      ...
      { id: "need", label: "Quel besoin important n’est pas suffisamment entendu ?", type: "text", hint: "Sécurité, reconnaissance, autonomie, expression, repos..." }
    ]
  },
  {
    title: "T — Trésor révélé",
    intro: "Explorer les deux polarités : ce qui a été difficile, mais aussi la sensibilité, la ressource et l’élan qui en émergent.",
    questions: [
...
function save() {
  try {
    ...
    if (saveStatus) saveStatus.textContent = "Enregistré sur cet appareil";
  } catch (e) {
    console.warn("Échec de la sauvegarde locale (quota ou navigation privée)", e);
  }
}
    ...
    return `
      <article class="question-card">
        ...
        <div class="chips" data-id="${q.id}">
          ${q.options.map(o => {
            const isSelected = val.includes(o);
            return `<button type="button" class="chip ${isSelected ? 'selected' : ''}" aria-pressed="${isSelected}" data-value="${escapeHtml(o)}">${escapeHtml(o)}</button>`;
          }).join("")}
        </div>
      </article>`;
  }
...
function renderStep() {
  ...
  $("#stepNumber").textContent = `Étape ${state.step + 1} sur ${STEPS.length}`;
  $("#progressBar").style.width = `${((state.step + 1) / STEPS.length) * 100}%`;
  
  const pause = state.step === 2 ? `
    <div class="pause-box">
      <span>♡</span>
  ...
  stepContent.innerHTML = `
    ...
    ${s.questions.map(question).join("")}
  `;

  stepContent.querySelectorAll("textarea").forEach(el => el.addEventListener("input", () => {
    state.answers[el.dataset.id] = el.value;
    save();
  }));

  stepContent.querySelectorAll('input[type="range"]').forEach(el => el.addEventListener("input", () => {
    state.answers[el.dataset.id] = +el.value;
    el.nextElementSibling.value = el.value;
    save();
  }));

  stepContent.querySelectorAll(".chips").forEach(group => group.querySelectorAll(".chip").forEach(chip => chip.onclick = () => {
    const id = group.dataset.id;
    const arr = state.answers[id] || [];
    const v = chip.dataset.value;
    const nextArr = arr.includes(v) ? arr.filter(x => x !== v) : [...arr, v];
    state.answers[id] = nextArr;
    chip.classList.toggle("selected");
    chip.setAttribute("aria-pressed", nextArr.includes(v));
    save();
  }));

  $("#previousBtn").style.visibility = state.step === 0 ? "hidden" : "visible";
  $("#nextBtn").innerHTML = state.step === STEPS.length - 1 ? 'Voir la synthèse <span>→</span>' : 'Continuer <span>→</span>';
}
...
function start(fresh = false) {
  if (fresh) {
    state = { step: 0, name: "", answers: {}, updatedAt: null };
    save();
  }
  const nameInput = $("#sessionName");
...
function renderSummary() {
  ...
  const intensityEvolution = hasIntensity ? `<div class="intensity-badge">Intensité ressentie : ${a.startIntensity}/10 → ${a.endIntensity}/10</div>` : "";
  ...
  $("#transformationCard").innerHTML = `
    ...
    <div class="insight">
      Cette carte est une piste formulée à partir des mots notés pendant la séance. Elle n’est juste que si elle résonne pour vous. 
      ${a.action ? `<br>Premier mouvement choisi : <strong>${escapeHtml(a.action)}</strong>` : ""}
    </div>`;

  $("#summaryContent").innerHTML = STEPS.map(s => `
...
$("#previousBtn").onclick = () => { collect(); if (state.step > 0) { state.step--; renderStep(); } };
$("#nextBtn").onclick = () => {
  collect();
